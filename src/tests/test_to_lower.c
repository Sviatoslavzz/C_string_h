...
START_TEST(hello_world_diy_func) {
  ...
  char* str_my_to_lower = my_to_lower(str_origin);
  ck_assert_str_eq(str_my_to_lower, str_to_upper_c);
  free(str_my_to_lower);
}
END_TEST
...
START_TEST(hello_world_str) {
  ...
  char* str_my_to_lower = my_to_lower(str_origin);
  ck_assert_str_eq(str_my_to_lower, str_check);
  free(str_my_to_lower);
}
END_TEST
